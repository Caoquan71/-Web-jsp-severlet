...
public class CategoryDBContext extends DBContext { //thao tác với bảng category
    ...
    public List<Category> getAllCategories() {
        ...
        try {
            String sql = "select * from Category_HE163447";
    ...
    public void insertCategory(String name) {
        try {
            String sql = "INSERT INTO [Category_HE163447]\n"
                    + "           ([cname])\n"
                    + "     VALUES\n"
                    + "           (?)";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public Category getCategoryById(int id) {
        try {
            String sql = "select *  from Category_HE163447 where cid = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
    ...
    public void updateCategory(Category category) {
        try {
            String sql = "UPDATE [dbo].[Category_HE163447]\n"
                    + "   SET [cname] = ?\n"
                    + " WHERE cid = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
