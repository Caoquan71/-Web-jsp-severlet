...
public class AcountDBContext extends DBContext {
    ...
    public List<Account> getAllAccount() {
        ...
        try {
            String sql = "SELECT * FROM Account_HE163447 where isAdmin != 1";
    ...
    public Account login(String user, String pass) {
        try {
            String sql = "SELECT * FROM Account_HE163447 where [user] = ? and pass = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public Account checkAccountExist(String user) {
        try {
            String sql = "SELECT * FROM Account_HE163447 where [user] = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, user);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
    ...
    public void insertAccount(String user, String pass) {
        try {
            String sql = "INSERT INTO [Account_HE163447]\n"
                    + "           ([user]\n"
                    + "           ,[pass]\n"
                    + "           ,[isSell]\n"
                    + "           ,[isAdmin]\n"
    ...
    public Account getAccountById(int accountId) {
        try {
            String sql = "select *  from Account_HE163447 where uID = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public void updateAccount(Account account) {
        ...
        try {
            String sql = "UPDATE [Account_HE163447]\n"
                    + "   SET [active] = ?\n"
                    + " WHERE uId = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
