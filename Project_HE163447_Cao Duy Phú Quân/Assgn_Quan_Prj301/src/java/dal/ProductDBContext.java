...
public class ProductDBContext extends DBContext {
    ...
    public List<Product> getAllProducts() {
        ...
        try {
            String sql = "select * from product_HE163447";
    ...
    public List<Product> getProductsByCategoryId(int categoryId) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from product_HE163447 where product_HE163447.cateID = ?";
    ...
    public List<Product> getProductsWithPagging(int page, int PAGE_SIZE) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from product_HE163447 order by id\n"
    ...
    public int getTotalProducts() {
        try {
            String sql = "select count(id)  from product_HE163447 ";
    ...
    public List<Product> search(String keyword) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from product_HE163447 where name like ?";
    ...
    public Product getProductById(int productId) {
        try {
            String sql = "select *  from product_HE163447 where id = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public List<Product> getProductsBySellId(int Id) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from product_HE163447 where sell_ID = ?";
    ...
    public void inSertProduct(Product product) {
        try {
            String sql = "INSERT INTO [product_HE163447]\n"
                    + "           ([name]\n"
                    + "           ,[image]\n"
                    + "           ,[price]\n"
                    + "           ,[title]\n"
    ...
    public void deleteProduct(int id) {
        try {
            String sql = "DELETE FROM [product_HE163447]\n"
                    + "WHERE id = ? ";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
            stm.executeUpdate();
    ...
    public void updateProduct(Product p) {
        try {
            String sql = "UPDATE [product_HE163447]\n"
                    + "   SET [name] = ?\n"
                    + "      ,[image] = ?\n"
                    + "      ,[price] = ?\n"
                    + "      ,[title] = ?\n"
    ...
    public List<Product> getAllProductsLast() {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT TOP 4 * FROM product_HE163447 ORDER BY ID DESC";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Product product = new Product();
